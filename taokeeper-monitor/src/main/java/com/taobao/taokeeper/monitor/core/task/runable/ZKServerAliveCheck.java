						...
						if ( !sub.checkIfAlive() ) {
							if ( !sub.checkIfAlive() ) { // 连续两次check失败
								...
								if ( GlobalInstance.needAlarm.get() ) {
									ThreadPoolManager.addJobToMessageSendExecutor( new GeneralMessageSender(alarmSettings, "ZooKeeper所在机器存活性检测失败" + this.zooKeeperCluster.getClusterName(), "Zk node: "
											+ server + " 存活性检测失败" ) );
								}
