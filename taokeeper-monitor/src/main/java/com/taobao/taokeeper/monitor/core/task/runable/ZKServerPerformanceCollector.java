			...
			if ( !StringUtil.isBlank( cpuUsage ) && cpuUsage.endsWith( "%" ) ) {
				...
				if ( 0 < difference ) {
					...
					if ( GlobalInstance.needAlarm.get() ) {
						ThreadPoolManager.addJobToMessageSendExecutor( new GeneralMessageSender( alarmSettings, "ZK Server cpu usage too high-" + clusterName, hostPerformanceEntity.getIp() + " cpu usage too high! " + cpuUsage + "-" + maxCpuUsage + "=" + difference
								) );
		...
		if ( !StringUtil.isBlank( maxMemoryUsage ) ) { // Memory usage alarm
			...
			if ( !StringUtil.isBlank( memoryUsage ) && memoryUsage.endsWith( "%" ) ) {
				...
				if ( 0 < difference ) {
					...
					// Alarm
					if ( GlobalInstance.needAlarm.get() ) {
						ThreadPoolManager.addJobToMessageSendExecutor( new GeneralMessageSender( alarmSettings, "ZK Server memory usage too high:-" + clusterName, hostPerformanceEntity.getIp() + " memory too high：" + memoryUsage + "-" + maxMemoryUsage + "=" + difference
								 ));
						LOG.info( "WangWangList: " + wangwangList );
					}
				}
			...
			if ( !StringUtil.isBlank( load ) ) {
				...
				if ( 0 < difference ) {
					...
					if ( GlobalInstance.needAlarm.get() ) {
						...
						ThreadPoolManager.addJobToMessageSendExecutor( new GeneralMessageSender( 
								alarmSettings, " ZK Server load usage too high-"
						...
						for ( String mountedOn : diskUsageMap.keySet() ) {
							...
							if ( dataDir.startsWith( StringUtil.trimToEmpty( mountedOn ) )
								...
								if ( diskUsage > Integer.parseInt( maxDiskUsage ) ) {
									...
									if ( GlobalInstance.needAlarm.get() ) {
										ThreadPoolManager.addJobToMessageSendExecutor( new GeneralMessageSender( alarmSettings,
