...
import com.taobao.taokeeper.reporter.alarm.MyMessageSender;
...
public class ZKServerStatusCollector implements Runnable {
	...
	private static final Logger LOG = LoggerFactory
			.getLogger(ZKServerStatusCollector.class);
	...
	public void run() {
		try {
			...
			sshZooKeeperAndHandleStat(ip, Integer.parseInt(port),
					zooKeeperStatus);
			telnetZooKeeperAndHandleWchs(ip, Integer.parseInt(port),
					zooKeeperStatus);
			sshZooKeeperAndHandleWchc(ip, Integer.parseInt(port),
					zooKeeperStatus, zookeeperCluster.getClusterId());
			sshZooKeeperAndHandleRwps(ip, Integer.parseInt(port),
					(ZooKeeperStatusV2) zooKeeperStatus,
					zookeeperCluster.getClusterId());
			checkAndAlarm(alarmSettings, zooKeeperStatus,
					zookeeperCluster.getClusterName());
			GlobalInstance.putZooKeeperStatus(ip, zooKeeperStatus);
			// Store taokeeper stat to DB
			if (needStoreToDB) {
				storeTaoKeeperStatToDB(zookeeperCluster.getClusterId(),
						(ZooKeeperStatusV2) zooKeeperStatus);
			}

			LOG.info("Finish #" + zookeeperCluster.getClusterName() + "-" + ip);
			LOG.info(ip + "rwps--zkserver run-"
					+ zooKeeperStatus.getRwps().toString());
	 ...
	 */
	private void sshZooKeeperAndHandleStat(String ip, int port,
		...
		try {
			sshResource = SSHUtil.executeWithoutHandleBufferedReader(
					ip,
					SystemConstant.portOfSSH,
					userNameOfSSH,
					passwordOfSSH,
					StringUtil.replaceSequenced(COMMAND_STAT, ip, port
							+ EMPTY_STRING));
			if (null == sshResource) {
				LOG.warn("No output of "
						+ StringUtil.replaceSequenced(COMMAND_STAT, ip, port
								+ EMPTY_STRING));
				return;
			}
			...
			while ((line = bufferedRead.readLine()) != null) {
				if (analyseLineIfClientConnection(line)) { // 检查是否是客户端连接
					...
					zooKeeperStatus
							.setNodeCount(Integer.parseInt(StringUtil
									.trimToEmpty(line.replace(NODE_COUNT,
											EMPTY_STRING))));
				} else if (line.contains(STRING_SENT)) {
					zooKeeperStatus.setSent(StringUtil.trimToEmpty(line
							.replace(STRING_SENT, EMPTY_STRING)));
				} else if (line.contains(STRING_RECEIVED)) {
					zooKeeperStatus.setReceived(StringUtil.trimToEmpty(line
							.replace(STRING_RECEIVED, EMPTY_STRING)));
				}
			...
			LOG.warn("Error when telnetZooKeeperAndHandleStat:[ip:" + ip
	 ...
	 * 进行Telnet连接并进行执行wchs。
	 */
	private void telnetZooKeeperAndHandleWchs(String ip, int port,
			ZooKeeperStatus zooKeeperStatus) {
		...
		try {
			...
			String wchsOutput = SSHUtil.execute(
					ip,
					SystemConstant.portOfSSH,
					userNameOfSSH,
					passwordOfSSH,
			...
			String firstLine = wchsOutputArray[0].replace(
					STRING_CONNECTIONS_WATCHING, WORD_SEPARATOR).replace(
			...
			int watchedPaths = Integer.parseInt(StringUtil
					.trimToEmpty(firstLineArray[1]));
			zooKeeperStatus.setConnections(consOfServer);
			zooKeeperStatus.setWatchedPaths(watchedPaths);

			// 分析第二行来获取watches数
			String secondtLine = wchsOutputArray[1].replace(
					STRING_TOTAL_WATCHES, EMPTY_STRING);
			int watches = Integer.parseInt(StringUtil.trimToEmpty(secondtLine));
			zooKeeperStatus.setWatches(watches);
		} catch (SSHException e) {
			LOG.warn("Error when telnetZooKeeperAndHandleWchs:[ip:" + ip
					+ ", port:" + port + " ] " + e.getMessage());
		} catch (Exception e) {
			LOG.error("程序出错：" + e.getMessage());
		}
	 ...
	 * @throws Exception
	 */
	private void sshZooKeeperAndHandleWchc(String ip, int port,
			ZooKeeperStatus zooKeeperStatus, int clusterId) {
		...
		try {
			...
			String wchcOutput = SSHUtil.execute(
					ip,
					SystemConstant.portOfSSH,
					userNameOfSSH,
					passwordOfSSH,
			...
			zooKeeperStatus.setWatchedPathMapContent(wchcOutputWithIp
					.toString());
		} catch (SSHException e) {
			LOG.warn("Error when sshZooKeeperAndHandleWchc:[ip:" + ip
					+ ", port:" + port + " ] " + e.getMessage());
		} catch (Exception e) {
	 ...
	 * @throws Exception
	 */
	private  void sshZooKeeperAndHandleRwps(String ip, int port,
		...
		try {
			...
			String rwpsOutput = SSHUtil.execute(
					ip,
					SystemConstant.portOfSSH,
					userNameOfSSH,
					passwordOfSSH,
					StringUtil.replaceSequenced(COMMAND_RWPS, ip, port
							+ EMPTY_STRING));
			LOG.info("rwps output=" + rwpsOutput);
			/**
			...
			LOG.info("wchcOutputArray=" + wchcOutputArray);
			for (String line : wchcOutputArray) {
				LOG.info("----------------------------line=" + line);
				if (StringUtil.isBlank(line)) {
					continue;
					...
					line = line.substring(line.indexOf("getChildren2")
							+ ("getChildren2".length() + 1));
					getChildren2 = Math.round(Double.valueOf(StringUtil
							.trimToEmpty(line)) * 100) / 100d;

				} else if (line.contains("createSession")) {

					line = line.substring(line.indexOf("createSession")
							+ ("createSession".length() + 1));
					createSession = Math.round(Double.valueOf(StringUtil
							.trimToEmpty(line)) * 100) / 100d;

				} else if (line.contains("closeSession")) {

					line = line.substring(line.indexOf("closeSession")
							+ ("closeSession".length() + 1));
					closeSession = Math.round(Double.valueOf(StringUtil
							.trimToEmpty(line)) * 100) / 100d;

				} else if (line.contains("setData")) {

					line = line.substring(line.indexOf("setData")
							+ ("setData".length() + 1));
					setData = Math.round(Double.valueOf(StringUtil
							.trimToEmpty(line)) * 100) / 100d;

				} else if (line.contains("setWatches")) {

					line = line.substring(line.indexOf("setWatches")
							+ ("setWatches".length() + 1));
					setWatches = Math.round(Double.valueOf(StringUtil
							.trimToEmpty(line)) * 100) / 100d;

				} else if (line.contains("getChildren")) {

					line = line.substring(line.indexOf("getChildren")
							+ ("getChildren".length() + 1));
					getChildren = Math.round(Double.valueOf(StringUtil
							.trimToEmpty(line)) * 100) / 100d;

				} else if (line.contains("delete")) {

					line = line.substring(line.indexOf("delete")
							+ ("delete".length() + 1));
					delete = Math.round(Double.valueOf(StringUtil
							.trimToEmpty(line)) * 100) / 100d;
					...
					line = line.substring(line.indexOf("create")
							+ ("create".length() + 1));
					create = Math.round(Double.valueOf(StringUtil
							.trimToEmpty(line)) * 100) / 100d;

				} else if (line.contains("exists")) {

					line = line.substring(line.indexOf("exists")
							+ ("exists".length() + 1));
					exists = Math.round(Double.valueOf(StringUtil
							.trimToEmpty(line)) * 100) / 100d;

				} else if (line.contains("getData")) {

					line = line.substring(line.indexOf("getData")
							+ ("getDate".length() + 1));
					getData = Math.round(Double.valueOf(StringUtil
							.trimToEmpty(line)) * 100) / 100d;
			...
			rwps = new ZooKeeperStatusV2.RWStatistics(getChildren2,
					createSession, closeSession, setData, setWatches,
					getChildren, delete, create, exists, getData);
			LOG.info("rwps collector----" + rwps.toString());
	...
	// 检查并进行报警
	private void checkAndAlarm(AlarmSettings alarmSettings,
			ZooKeeperStatus zooKeeperStatus, String clusterName) {
		...
		try {
			...
			String maxConnectionPerIp = StringUtil.trimToEmpty(alarmSettings
					.getMaxConnectionPerIp());
			String maxWatchPerIp = StringUtil.trimToEmpty(alarmSettings
					.getMaxWatchPerIp());

			if (!StringUtil.isBlank(maxConnectionPerIp)) {
				Map<String, Connection> conns = zooKeeperStatus
						.getConnections();
				...
				if (Integer.parseInt(maxConnectionPerIp) < connectionsPerIp) {
					needAlarm = true;
					sb.append(zooKeeperStatus.getIp() + " 上的连接数达到了: "
							+ connectionsPerIp + ", 超过设置的报警阀值: "
							+ maxConnectionPerIp + ".  ");
				}
			}
			...
			if (!StringUtil.isBlank(maxWatchPerIp)) {
				...
				if (Integer.parseInt(maxWatchPerIp) < watchesPerIp) {
					needAlarm = true;
					sb.append(zooKeeperStatus.getIp() + " 上的Watch数达到了: "
							+ watchesPerIp + ", 超过设置的报警阀值: " + maxWatchPerIp
							+ ".  ");
				}
			}
...
//					ThreadPoolManager
//							.addJobToMessageSendExecutor(new TbMessageSender(
//									new Message(wangwangList,
//											"ZooKeeper连接数，Watcher数报警-"
//													+ clusterName, clusterName
//													+ "-" + sb.toString(),
//											Message.MessageType.WANGWANG)));
					LOG.warn("begin send message---------------------");
					ThreadPoolManager.addJobToMessageSendExecutor(new MyMessageSender());
					ThreadPoolManager
							.addJobToMessageSendExecutor(new GeneralMessageSender(
									alarmSettings,
											"ZooKeeper连接数，Watcher数报警-"
													+ clusterName, clusterName
													+ "-" + sb.toString()));
	 ...
	 */
	private void storeTaoKeeperStatToDB(int clusterId,
			ZooKeeperStatusV2 zooKeeperStatus) {
		...
		try {
			...
			if (!ObjectUtil.isBlank(zooKeeperStatus.getRwps())) {
				rwStatistics = new Gson().toJson(zooKeeperStatus.getRwps(),
						type.getType());
			}
			if (zooKeeperStatus.getReceived() == null
					|| "".equals(zooKeeperStatus.getReceived()))
				zooKeeperStatus.setReceived("0");
			if (zooKeeperStatus.getSent() == null
					|| "".equals(zooKeeperStatus.getSent()))
				zooKeeperStatus.setSent("0");
			reportDAO.addTaoKeeperStat(new TaoKeeperStat(clusterId,
					zooKeeperStatus.getIp(), DateUtil
							.getNowTime(DateFormat.DateTime), DateUtil
							.getNowTime(DateFormat.Date), MapUtil
							.size(zooKeeperStatus.getConnections()),
					zooKeeperStatus.getWatches(), Long
							.parseLong(zooKeeperStatus.getSent()),
	...
	public static void main(String args[]) {
		ZooKeeperStatusV2 zs = new ZooKeeperStatusV2();
		//sshZooKeeperAndHandleRwps("z1.test.com",2181,zs,0);
		//String rwpsOutput = "RealTime R/W Statistics:<br/>      getChildren2:       0.0<br/><br/>   createSession:  0.09999000099990002<br/><br/>   closeSession:   0.0<br/><br/>   setData:        0.09999000099990002<br/><br/>       setWatches:     0.0<br/><br/>   getChildren:    0.0<br/><br/>   delete:     0.0<br/><br/>       create:         0.0<br/><br/>       exists:         0.29997000299970006<br/><br/>   getData:        0.09999000099990002<br/><br/>";
		String rwpsOutput="RealTime R/W Statistics:<br/>      getChildren2:       0.0<br/><br/>   createSession:  0.09999000099990002<br/><br/>   closeSession:   0.0<br/><br/>   setData:        0.09999000099990002<br/><br/>       setWatches:     0.0<br/><br/>   getChildren:    0.0<br/><br/>   delete:     0.0<br/><br/>       create:         0.0<br/><br/>       exists:         0.29997000299970006<br/><br/>   getData:        9.799020097990201<br/><br/>";
		String[] wchcOutputArray = rwpsOutput.split(BR);
		if (0 == wchcOutputArray.length) {
			LOG.warn("No output of command "
					);
		...
		System.out.println(StringUtil
				.trimToEmpty("9.799020097990201"));
		System.out.println(Double.valueOf(StringUtil
				.trimToEmpty("9.799020097990201")) * 100);
		System.out.println(Math.round(Double.valueOf(StringUtil
				.trimToEmpty("9.799020097990201")) * 100));
		getData = Math.round(Double.valueOf(StringUtil
				.trimToEmpty("9.799020097990201")) * 100) / 100;
